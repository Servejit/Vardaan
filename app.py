...
def logout():
    try:
        ...
        pass
    for k in ["user", "profile", "is_admin", "recovery"]:
        st.session_state[k] = None if k in ["user", "profile"] else False
    st.rerun()
...
if "code" in st.query_params and not st.session_state.recovery:
    try:
    ...
    if login:
        try:
            ...
            if not profile:
                ...
                st.session_state.is_admin = profile["role"] == "admin"
                st.rerun()
        except Exception as e:
...
if data:
    ...
    st.warning("Master Excel not uploaded yet.")
...
if is_admin:
    ...
    if upload and st.button("Save Master Excel", use_container_width=True):
        try:
            save_file(upload.getvalue())
            st.success("Master Excel saved successfully.")
            st.rerun()
        except Exception as e:
            st.error(str(e))
    ...
    if data:
        ...
        try:
            ...
            cell = st.text_input("Cell address", "A1")
            ...
            if st.button("Save Cell Change", use_container_width=True):
                ws[cell].value = new_value

                output = io.BytesIO()
                wb.save(output)
                save_file(output.getvalue())

                st.success(
                    f"{sheet}!{cell} updated. "
                    "Excel formatting was preserved."
                )
                st.rerun()
    ...
    if data:
        if st.button("Delete Master Excel", use_container_width=True):
            try:
                delete_file()
    ...
    try:
        users = admin.table("profiles").select("*").order(
            "email"
        ).execute().data
        ...
        for u in users:
            ...
            if c2.button("Role", key=f"role_{uid}"):
                new = "user" if u["role"] == "admin" else "admin"
            ...
            if c4.button("Delete", key=f"delete_{uid}"):
                if uid == user.id:
...
# ============================================================
    ...
    st.info("USER ACCESS â€” Read-only")
